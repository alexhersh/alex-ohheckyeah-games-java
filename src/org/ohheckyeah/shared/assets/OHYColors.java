package org.ohheckyeah.shared.assets;

import com.haxademic.core.draw.color.ColorUtil;

public class OHYColors {
	
	public final static int LOGO_COLOR = ColorUtil.colorFromHex("#000000");
	public final static int MAIN_TEXT_COLOR = ColorUtil.colorFromHex("#000000");
	public final static int INTRO_SCREENS_BG = ColorUtil.colorFromHex("#ffffff");
	public final static int INTRO_SCREENS_BG_BLUE = ColorUtil.colorFromHex("#82d2f5");
	
}
